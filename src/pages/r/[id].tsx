...
const Index: NextPage = () => {
  ...
  if (!room || typeof room !== "string") return <div></div>;
  ...
  return (
    <div className="flex h-screen w-screen flex-col items-center justify-center bg-slate-800">
      <div className="pt-4 text-4xl capitalize">{room}</div>
      <Room client={pusherClient} roomId={room.replaceAll("/", "")}></Room>
    </div>
  );
};
...
const VideoRTC: React.FC<{ props?: any }> = ({ props }) => {
  ...
  return (
    <div
      className={`relative mt-2 flex w-[${props.ratio}rem] justify-center overflow-hidden rounded-2xl border-4 border-slate-500 bg-slate-500`}
    >
